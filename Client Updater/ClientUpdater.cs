...
{
    public class ClientUpdater : Constants
    {
        ...
        public ClientUpdater(string domain, Label update)
        {
            this.domain = domain;
            label = update;
        }
        ...
        private void ExportPacketIds()
        {
            ...
            string targetFile = Empty;
            string packetIdText =
@"namespace wServer
{
    public enum PacketID : byte
    {";
            var PacketIdCollection = new Dictionary<int, KeyValuePair<string, int>>();
    ...
    @"
                ...
                using (var writer = new StreamWriter(Environment.CurrentDirectory + "\\PacketIds.cs", false))
                    writer.Write(packetIdText);
            }
        ...
        private void Decompile()
        {
            ...
            var process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\swfdecompress.exe", "client.swf"));
            process.WaitForExit();

            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\abcexport.exe", "client.swf"));
            process.WaitForExit();

            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\rabcdasm.exe", "client-1.abc"));
            process.WaitForExit();

            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\swfbinexport.exe", "client.swf"));
            process.WaitForExit();

            files = Directory.GetFiles(Environment.CurrentDirectory + @"\client-1", "*.class.asasm", SearchOption.AllDirectories);
        }
        ...
        private void Recompile()
        {
            ...
            var process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\rabcasm.exe", "client-1\\client-1.main.asasm"));
            process.WaitForExit();

            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\abcreplace.exe", "client.swf 1 client-1\\client-1.main.abc"));
            process.WaitForExit();

            process = Process.Start(Environment.CurrentDirectory + "\\Orape.exe");
            MessageBox.Show("Now Build your client with orape and press ok after orape is done to compress the client");

            while (!process.HasExited) MessageBox.Show("Orape is still running, close it before you continue");
            ...
            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\swflzmacompress.exe", "client.swf"));
            process.WaitForExit();

            process = Process.Start(new ProcessStartInfo(Environment.CurrentDirectory + @"\rabcdasm\swflzmacompress.exe", "client-mod.swf"));
            process.WaitForExit();
        ...
        private void ChangeCopyright()
        {
            ...
            foreach (var file in files)
            {
                ...
                using (var writer = new StreamWriter(file.Key, false))
        ...
        {
            label.Text = Format("Status: {0}", message);
            label.Update();
        }
            ...
            foreach (var bin in bins)
            {
                try
                {
                    unsafe
                    {
                        ...
                        using (var fullxml = new StreamWriter("dat1.xml"))
                            fullxml.WriteLine(objectxml + "</Objects>");

                        using (var fullxml = new StreamWriter("dat0.xml"))
                            fullxml.WriteLine(groundxml + "</GroundTypes>");

                        using (var fullxml = new StreamWriter("EquipmentSets.xml"))
